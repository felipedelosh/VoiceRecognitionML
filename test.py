...
print(f"MAX FRAMES: {MAX_AUDIO_LEN}")
print(_txt)
_LOG = _LOG + _txt + "\n"
...
def extract_features(y, sr, max_len):
    ...
    mfccs = librosa.util.fix_length(mfccs.T, size=max_len, axis=0)  # Transponer antes de ajustar la longitud
    return mfccs
...
_model_max_len = _shape_audio_input[1]  # Ajustado para obtener la longitud máxima
_model_n_mfcc = _shape_audio_input[2]  # Ajustado para obtener el número de características
transcripts = np.array(transcripts)
print("==============   STEP 3 of X  NEURAL INPUT INFORMATION ==================")
print(f"Audio data shape: {_shape_audio_input}")
print(f"Model INPUT: {_model_max_len, _model_n_mfcc}")
...
max_len_transcripts = _model_max_len  # Ajustar a la misma longitud que la salida del modelo
...
# Definir el modelo
model = Sequential()
model.add(Input(shape=(_model_max_len, _model_n_mfcc)))  # max_len primero, n_mfcc después
...
if len(audio_data) == 0 or len(padded_transcripts) == 0:
    print("No hay suficientes datos para el entrenamiento.")
    _LOG = _LOG + "No hay suficientes datos para el entrenamiento.\n"
else:
